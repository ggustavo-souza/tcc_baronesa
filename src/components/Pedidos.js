...
// Importação de useCallback adicionada aqui!
...
function MeusPedidos() {
	...
	// Função de MÁSCARA encapsulada em useCallback (para ser estável)
	...
	// Handler para o campo CPF/CNPJ, aplicando a máscara
	...
	// Função utilitária para limpar e validar CPF/CNPJ (mantida, mas não usa máscara)
	function formatAndCleanCpf(value) {
		return value.replace(/\D/g, '');
	}

	// Função para formatar como moeda BRL (mantida)
	...
	// carregarPedidos encapsulada em useCallback para resolver o aviso do Linter (Vercel)
	const carregarPedidos = useCallback(async (idUsuario) => {
		if (!idUsuario) {
			...
			setModal(true)
			setLoading(false); // Adicionado para parar o loading se não houver ID
			return;
		}

		try {
			const resposta = await fetch(`${urlAPI}/api/pedidos/${idUsuario}`);
			if (!resposta.ok) { // Trata erros HTTP como 404/500
	...
	// useEffect CORRIGIDO com dependências para resolver o problema da Vercel
	useEffect(() => {
		const usuario = JSON.parse(localStorage.getItem('usuarioLogado'));
		if (usuario) {
			setUsuarioLogado(usuario);
			carregarPedidos(usuario.id); // carregarPedidos é a dependência
		} else {
			setLoading(false);
		}
		Aos.init({ duration: 600, once: true });
	}, [carregarPedidos, setUsuarioLogado, setLoading]); // carregarPedidos, setUsuarioLogado e setLoading ADICIONADOS
	...
	async function handleConfirmPayment() {
		...
		try {
			...
			// Sucesso: Redirecionar para o Checkout Pro (Retornando ao estado funcional original)
			window.location.href = data.init_point;
			setLoading(false); // apenas por segurança, mas o redirecionamento já encerra
								...
								<div className="modal-body py-4 bg-white">
									...
									<div className="mb-3">
										...
										<input
											...
											className="form-control"
											value={cpfInput}
											// AQUI: Usa o novo handler para aplicar a máscara
											onChange={handleCpfCnpjChange}
											// AQUI: Máximo para CPF/CNPJ com pontos e traços
											maxLength={18}
