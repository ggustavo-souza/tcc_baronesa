...
export default function MoveisCrud() {
    ...
    async function carregarMoveis() {
        try {
            const res = await fetch(`${urlAPI}/api/moveis`);
    ...
    async function deletarMovel() {
        ...
        try {
            const res = await fetch(`${urlAPI}/api/moveis/${movelIdParaExcluir}`, { method: 'DELETE' });
                                ...
                                <tbody>
                                    {registros.map(m => (
                                        <tr key={m.id}>
                                            ...
                                            <td>
                                                {m.fotos?.length > 0 && (
                                                    <img src={`${urlAPI}/api/uploads/${m.fotos.find(f => f.principal)?.foto || m.fotos[0].foto}`} alt={m.nome} width="50" className="img-thumbnail" />
                                                )}
                                            </td>
                                            <td>
